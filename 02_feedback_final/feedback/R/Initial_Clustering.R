...
#' @importFrom SimilaR SimilaR_fromDirectory
#'
...
initial_cluster <- function(sol_path = ".",
  ...
  # If there are more than one professor solutions.
  if(length(sol_files_names) > 1) {
    ...
    while(nrow(prof_mat) != 0) {
      ...
      # The indices of the similarity matrix containing the current prof
      # solutions
      inds <- which(sim_mat$name1 %in% curr_prof_sols
                    | sim_mat$name2 %in% curr_prof_sols)

      curr_sim_mat <- sim_mat[inds,]
      curr_sim_mat <- curr_sim_mat[which(curr_sim_mat$SimilaR >
                                           curr_threshold),]
